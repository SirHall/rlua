...
impl<'lua> Context<'lua>
{
    ...
    ///
    /// In order to actually compile or run the resulting code, you must call
    /// [`Chunk::exec`] or similar on the returned builder.  Code is not
    /// even parsed until one of these methods is called.
    ///
    ...
    /// Create and return an interned Lua string.  Lua strings can be arbitrary
    /// [u8] data including embedded nulls, so in addition to `&str` and
    /// `&String`, you can also pass plain `&[u8]` here.
        ...
        I : IntoIterator<Item = (K, V)>,
    {
        unsafe {
            let _sg = StackGuard::new(self.state);
            // `Lua` instance assumes that on any callback, the Lua stack has at least
            // LUA_MINSTACK slots available to avoid panics.
    ...
    /// Wraps a Rust function or closure, creating a callable Lua function
    /// handle to it.
    ///
    /// The function's return value is always a `Result`: If the function
    /// returns `Err`, the error is raised as a Lua error, which can be
    /// caught using `(x)pcall` or bubble up to the Rust code that invoked
    /// the Lua code. This allows using the `?` operator to propagate errors
    /// through intermediate Lua code.
    ///
    /// If the function returns `Ok`, the contained value will be converted to
    /// one or more Lua values. For details on Rust-to-Lua conversions,
    /// refer to the [`ToLua`] and [`ToLuaMulti`] traits.
    ///
    /// # Examples
    ///
    ...
    /// Wraps a Rust mutable closure, creating a callable Lua function handle to
    /// it.
    ///
    /// This is a version of [`create_function`] that accepts a FnMut argument.
    /// Refer to [`create_function`] for more information about the
    /// implementation.
    ///
        ...
        F : 'static + Send + FnMut(Context<'lua>, A) -> Result<R>,
    {
        ...
        self.create_function(move |lua, args| {
            (&mut *func.try_borrow_mut().map_err(|_| Error::RecursiveMutCallback)?)(lua, args)
        })
    }
    ...
    pub fn create_thread(self, func : Function<'lua>) -> Result<Thread<'lua>>
    {
        unsafe {
            ...
            let thread_state = protect_lua_closure(self.state, 0, 1, |state| ffi::lua_newthread(state))?;
    ...
    /// Returns a handle to the active `Thread` for this `Context`.  For calls
    /// to `Lua::context` this will be the main Lua thread, for `Context`
    /// parameters given to a callback, this will be whatever Lua thread
    /// called the callback.
    ...
    /// Calls the given function with a `Scope` parameter, giving the function
    /// the ability to create userdata and callbacks from rust types that
    /// are !Send or non-'static.
    ///
    /// The lifetime of any function or userdata created through `Scope` lasts
    /// only until the completion of this method call, on completion all
    /// such created values are automatically dropped and Lua references to
    /// them are invalidated.  If a script accesses a value created
    /// through `Scope` outside of this method, a Lua error will result.  Since
    /// we can ensure the lifetime of values created through `Scope`, and we
    /// know that `Lua` cannot be sent to another thread while `Scope` is
    /// live, it is safe to allow !Send datatypes and whose lifetimes only
    /// outlive the scope lifetime.
    ///
    /// Inside the scope callback, all handles created through Scope will share
    /// the same unique 'lua lifetime of the parent `Context`.  This allows
    /// scoped and non-scoped values to be mixed in API calls, which is very
    /// useful (e.g. passing a scoped userdata to a non-scoped function).
    /// However, this also enables handles to scoped values to be trivially
    /// leaked from the given callback. This is not dangerous, though!
    /// After the callback returns, all scoped values are invalidated, which
    /// means that though references may exist, the Rust types backing them have
    /// dropped.  `Function` types will error when called, and `AnyUserData`
    /// will be typeless.  It would be impossible to prevent handles to
    /// scoped values from escaping anyway, since you would always be able
    /// to smuggle them through Lua state.
    ...
    /// Attempts to coerce a Lua value into a String in a manner consistent with
    /// Lua's internal behavior.
    ///
    /// To succeed, the value must be a string (in which case this is a no-op),
    ...
    pub fn coerce_string(self, v : Value<'lua>) -> Result<Option<String<'lua>>>
    {
        Ok(match v
        {
            ...
            unsafe {
                ...
                self.push_value(v)?;
                if protect_lua_closure(self.state, 1, 1, |state| !ffi::lua_tostring(state, -1).is_null())?
                {
    ...
    /// Attempts to coerce a Lua value into an integer in a manner consistent
    /// with Lua's internal behavior.
    ///
    /// To succeed, the value must be an integer, a floating point number that
    /// has an exact representation as an integer, or a string that can be
    /// converted to an integer. Refer to the Lua manual for details.
    ...
    /// Attempts to coerce a Lua value into a Number in a manner consistent with
    /// Lua's internal behavior.
    ///
    /// To succeed, the value must be a number or a string that can be converted
    /// to a number. Refer to the Lua manual for details.
    ...
    pub fn pack<T : ToLua<'lua>>(self, t : T) -> Result<Value<'lua>> { t.to_lua(self) }

    /// Converts a `Value` instance into a value that implements `FromLua`.
    pub fn unpack<T : FromLua<'lua>>(self, value : Value<'lua>) -> Result<T> { T::from_lua(value, self) }

    /// Converts a value that implements `ToLuaMulti` into a `MultiValue`
    /// instance.
    pub fn pack_multi<T : ToLuaMulti<'lua>>(self, t : T) -> Result<MultiValue<'lua>> { t.to_lua_multi(self) }

    /// Converts a `MultiValue` instance into a value that implements
    /// `FromLuaMulti`.
    ...
    ///
    /// This value will be available to rust from all `Lua` instances which
    /// share the same main state.
    ...
    ///
    /// Any Lua instance which shares the underlying main state may call this
    /// method to get a value previously set by
    ...
    /// share the same main state.
    ///
    /// The returned [`RegistryKey`] is of `'static` lifetime and is *the* main
    /// way in `rlua` of maintaining ownership of a Lua value outside of a
    /// [`Lua::context`] call.
    ///
    /// Be warned, garbage collection of values held inside the registry is not
    /// automatic, see [`RegistryKey`] for more details.
    ///
    ...
    pub fn create_registry_value<T : ToLua<'lua>>(self, t : T) -> Result<RegistryKey>
    {
        ...
        unsafe {
            ...
            self.push_value(t)?;
            let registry_id =
                protect_lua_closure(self.state, 1, 0, |state| ffi::luaL_ref(state, ffi::LUA_REGISTRYINDEX))?;
    ...
    ///
    /// Any Lua instance which shares the underlying main state may call this
    /// method to get a value previously placed by
    ...
    pub fn registry_value<T : FromLua<'lua>>(self, key : &RegistryKey) -> Result<T>
    {
        let value = unsafe {
            ...
            #[cfg(any(rlua_lua53, rlua_lua54))]
            ffi::lua_rawgeti(self.state, ffi::LUA_REGISTRYINDEX, key.registry_id as ffi::lua_Integer);
            #[cfg(any(rlua_lua51))]
    ...
    ///
    /// You may call this function to manually remove a value placed in the
    /// registry with [`create_registry_value`]. In addition to manual
    /// `RegistryKey` removal, you can also call [`expire_registry_values`]
    ...
    /// Returns true if the given `RegistryKey` was created by a `Lua` which
    /// shares the underlying main state with this `Lua` instance.
    ///
    /// Other than this, methods that accept a `RegistryKey` will return
    /// `Error::MismatchedRegistryKey` if passed a `RegistryKey` that was not
    /// created with a matching `Lua` state.
    pub fn owns_registry_value(self, key : &RegistryKey) -> bool
    {
        unsafe { Arc::ptr_eq(&key.unref_list, &(*extra_data(self.state)).registry_unref_list) }
    }

    /// Remove any registry values whose `RegistryKey`s have all been dropped.
    ///
    /// Unlike normal handle values, `RegistryKey`s do not automatically remove
    /// themselves on Drop, but you can call this method to remove any
    /// unreachable registry values not manually removed
    ...
    pub(crate) unsafe fn pop_value(self) -> Value<'lua>
    {
        match ffi::lua_type(self.state, -1)
        {
            ...
            ffi::LUA_TUSERDATA =>
            {
                ...
                // UserData types OR a WrappedError.  WrappedPanic should never be able to be
                // caught in lua, so it should never be here.
    ...
    // Pushes a LuaRef value onto the stack, uses 1 stack space, does not call
    // checkstack
    ...
    // Pops the topmost element of the stack and stores a reference to it.  This
    // pins the object, preventing garbage collection until the returned
    // `LuaRef` is dropped.
    //
    // References are stored in the stack of a specially created auxiliary thread
    // that exists only to store reference values.  This is much faster than
    // storing these in the registry, and also much more flexible and requires
    // less bookkeeping than storing them directly in the currently used stack.
    // The implementation is somewhat biased towards the use case of a relatively
    // small number of short term references being created, and `RegistryKey`
    // being used for long term references.
    ...
    {
        if let Some(table_id) = (*extra_data(self.state)).registered_userdata.get(&TypeId::of::<T>())
        {
            return Ok(*table_id);
        }
        ...
        let id = protect_lua_closure(self.state, 1, 0, |state| ffi::luaL_ref(state, ffi::LUA_REGISTRYINDEX))?;
    ...
    // This function is safe because the callbacks here are 'static, and the 'lua
    // context lifetime used in the callback parameters is not user chosen which
    // prevents the user from making 'lua grow to become 'static.  The lifetime
    // of the callback parameters here is a convenient lie to get around the
    // fact that without ATCs, we cannot easily deal with the "correct" type of
    // `Callback`, which is:
    //
    // Box<for<'lua> Fn(Context<'lua>, MultiValue<'lua>) ->
    // Result<MultiValue<'lua>>)>
    //
    // When ATCs become available in Rust, the signature of the ToLua / FromLua
    // traits should be changed to remove the lifetime parameter, which will
    // enable using the correct callback type and will reduce the number of
    // hacks required in Context and Scope.
    pub(crate) fn create_callback(self, func : Callback<'lua, 'static>) -> Result<Function<'lua>>
    {
        ...
        unsafe {
            ...
            ffi::lua_pushlightuserdata(self.state, &FUNCTION_METATABLE_REGISTRY_KEY as *const u8 as *mut c_void);
            ffi::lua_rawget(self.state, ffi::LUA_REGISTRYINDEX);
            ffi::lua_setmetatable(self.state, -2);

            protect_lua_closure(self.state, 1, 1, |state| {
                // The function we actually push to Lua is a wrapper that calls our Callback
                // instance which refers to our original function
    ...
    ) -> Result<Function<'lua>>
    {
        unsafe {
            ...
            let mode = if allow_binary { cstr!("bt") } else { cstr!("t") };
...
/// Returned from [`Context::load`] and is used to finalize loading and
/// executing Lua main chunks.
///
...
{
    /// Sets the name of this chunk, which results in more informative error
    /// traces.
    pub fn set_name<S : ?Sized + AsRef<[u8]>>(mut self, name : &S) -> Result<Chunk<'lua, 'a>>
    {
        let name = CString::new(name.as_ref().to_vec()).map_err(|e| Error::ToLuaConversionError {
            from :    "&str",
            to :      "string",
    ...
    ///
    /// Lua main chunks always have exactly one upvalue, and this upvalue is
    /// used as the `_ENV` variable inside the chunk.  By default this value
    /// is set to the global environment.
    ///
    /// Calling this method changes the `_ENV` upvalue to the value provided,
    /// and variables inside the chunk will refer to the given environment
    /// rather than the global one.
    ///
    /// All global variables (including the standard library!) are looked up in
    /// `_ENV`, so it may be necessary to populate the environment in order
    /// for scripts using custom environments to be useful.
    ...
    ///
    /// This is equivalent to calling the chunk function with no arguments and
    /// no return values.
    ...
    ///
    /// If the chunk can be parsed as an expression, this loads and executes the
    /// chunk and returns the value that it evaluates to.  Otherwise, the
    /// chunk is interpreted as a block as normal, and this is equivalent to
    /// calling `exec`.
    ...
    ///
    /// This is equivalent to `into_function` and calling the resulting
    /// function.
    ...
    {
        self.context.load_chunk(self.source, self.name.as_ref(), self.env, true)
    }
}
...
unsafe fn ref_stack_pop(extra : *mut ExtraData) -> c_int
{
    ...
    else
    {
        ...
        {
            // It is a user error to create enough references to exhaust the Lua max stack
            // size for the ref thread.
...
impl<'lua, T : 'static + UserData> UserDataMethods<'lua, T> for StaticUserDataMethods<'lua, T>
{
    ...
    {
        self.methods.push((name.as_ref().to_vec(), Self::box_method(method)));
    }
    ...
    where
        ...
        F : 'static + Send + FnMut(Context<'lua>, A) -> Result<R>,
    {
        self.meta_methods.push((meta, Self::box_function_mut(function)));
    }
}
...
impl<'lua, T : 'static + UserData> StaticUserDataMethods<'lua, T>
{
        ...
        F : 'static + Send + FnMut(Context<'lua>, A) -> Result<R>,
    {
        ...
        Box::new(move |lua, args| {
            let function = &mut *function.try_borrow_mut().map_err(|_| Error::RecursiveMutCallback)?;
