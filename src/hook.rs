...
///
/// The `Debug` structure is provided as a parameter to the hook function set
/// with [`Lua::set_hook`].  You may call the methods on this structure to
/// retrieve information about the Lua code executing at the time that the hook
/// function was called.  Further information can be found in the [Lua 5.4
/// documentaton][lua_doc].
///
...
impl<'a> Debug<'a>
{
    ...
    /// Corresponds to the `t` what mask. Returns true if the hook is in a
    /// function tail call, false otherwise.
...
pub struct HookTriggers
{
    ...
    pub every_line :            bool,
    /// After a certain number of VM instructions have been executed.  When set
    /// to `Some(count)`, `count` is the number of VM instructions to
    /// execute before calling the hook.
    ///
    /// # Performance
    ///
...
impl HookTriggers
{
    ...
    // Returns the `count` parameter to pass to `lua_sethook`, if applicable.
