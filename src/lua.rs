...
impl Lua
{
    ...
    /// library.
    pub fn new() -> Lua { unsafe { create_lua(StdLib::ALL_NO_DEBUG, InitFlags::DEFAULT) } }

    /// Creates a new Lua state and loads the standard library including the
    /// `debug` library.
    ///
    /// The debug library is very unsound, it can be used to break the safety
    /// guarantees of rlua.
    pub unsafe fn new_with_debug() -> Lua { create_lua(StdLib::ALL, InitFlags::DEFAULT) }
    ...
    ///
    /// Note that the `debug` library can't be loaded using this function as it
    /// can be used to break the safety guarantees of rlua.  If you really
    /// want to load it, use the sister function [`Lua::unsafe_new_with`].
    ///
    /// # Panics
    ///
    ...
    /// library which can be used to break the safety guarantees provided by
    /// rlua.
    pub unsafe fn unsafe_new_with(lua_mod : StdLib) -> Lua { create_lua(lua_mod, InitFlags::DEFAULT) }
    ...
    /// This function is unsafe because it can be used to load the `debug`
    /// library which can be used to break the safety guarantees provided by
    /// rlua, or to disable some of the safety features which rlua provides
    /// by default.
    ...
    /// Loads the specified set of safe standard libraries into an existing Lua
    /// state.
    ///
    /// Use the [`StdLib`] flags to specifiy the libraries you want to load.
    ///
    /// Note that the `debug` library can't be loaded using this function as it
    /// can be used to break the safety guarantees of rlua.  If you really
    /// want to load it, use the sister function
    ...
    /// Loads the specified set of standard libraries into an existing Lua
    /// state.
    ///
    /// Use the [`StdLib`] flags to specifiy the libraries you want to load.
    ///
    ...
    ///
    /// In order to create Lua values, load and execute Lua code, or otherwise
    /// interact with the Lua state in any way, you must first call
    /// `Lua::context` and then call methods on the provided [`Context`]
    /// parameter.
    ///
    /// rlua uses reference types like `String` and `Table` which reference
    /// shared data in the Lua state.  These are special reference counted
    /// types that contain pointers to the main Lua state via the
    /// [`Context`] type, and there is a `'lua` lifetime associated with these.
    ///
    /// This `'lua` lifetime is somewhat special.  It is what is sometimes
    /// called a "generative" lifetime or a "branding" lifetime, which is
    /// invariant, and unique for each call to `Lua::context`.
    ///
    /// The reason this entry point must be a callback is so that this unique
    /// lifetime can be generated as part of the callback's parameters.
    /// Even though this callback API is somewhat inconvenient, it has
    /// several advantages:
    ///
    /// - Inside calls to `Lua::context`, we know that all instances of the 'lua
    ///   lifetime are the same unique lifetime.  Thus, it is impossible for the
    ///   user to accidentally mix handle types between different instances of
    ///   `Lua`.
    /// - Because we know at compile time that handles cannot be mixed from
    ///   different instances of `Lua`, we do not need to do runtime checks to
    ///   make sure that handles are from the same state.
    /// - Handle types cannot escape the context call and the `'lua` context
    ///   lifetime is in general very limited, preventing it from being stored
    ///   in unexpected places.  This is a benefit as it helps ensure the
    ///   soundness of the API.
    ///
    /// It is not possible to return types with this `'lua` context lifetime
    /// from the given callback, or store them outside of the callback in
    /// any way.  There is an escape hatch here, though: if you need to keep
    /// references to internal Lua values long-term, you can use the Lua
    /// registry via [`Context::set_named_registry_value`] and
    ...
    /// [`Context::set_named_registry_value`]:
    /// struct.Context.html#method.set_named_registry_value [`Context::
    /// create_registry_value`]:
    /// struct.Context.html#method.create_registry_value
    ...
    /// Sets a 'hook' function that will periodically be called as Lua code
    /// executes.
    ///
    /// When exactly the hook function is called depends on the contents of the
    /// `triggers` parameter, see [`HookTriggers`] for more details.
    ///
    /// The provided hook function can error, and this error will be propagated
    /// through the Lua code that was executing at the time the hook was
    /// triggered.  This can be used to implement a limited form of
    /// execution limits by setting [`HookTriggers.every_nth_instruction`] and
    ...
    /// [`HookTriggers.every_nth_instruction`]:
    /// struct.HookTriggers.html#field.every_nth_instruction
        ...
        F : 'static + Send + FnMut(Context, Debug) -> Result<()>,
    {
        unsafe {
            ...
            ffi::lua_sethook(self.main_state, Some(hook_proc), triggers.mask(), triggers.count());
        }
    }

    /// Remove any hook previously set by `set_hook`. This function has no
    /// effect if a hook was not previously set.
    ...
    pub fn used_memory(&self) -> usize { unsafe { (*extra_data(self.main_state)).used_memory } }

    /// Sets a memory limit on this Lua state.  Once an allocation occurs that
    /// would pass this memory limit, a `Error::MemoryError` is generated
    /// instead.
    ...
    /// Returns true if the garbage collector is currently running
    /// automatically.
    pub fn gc_is_running(&self) -> bool { unsafe { ffi::lua_gc(self.main_state, ffi::LUA_GCISRUNNING, 0) != 0 } }
    ...
    ///
    /// It may be necessary to call this function twice to collect all currently
    /// unreachable objects.  Once to finish the current gc cycle, and once
    /// to start and finish the next cycle.
    ...
    ///
    /// if `kbytes` is 0, then this is the same as calling `gc_step`.  Returns
    /// true if this step has finished a collection cycle.
    ...
    ///
    /// Returns the previous mode (`LUA_GCGEN` or `LUA_GCINC`).  More
    /// information can be found in the [Lua 5.4 documentation][lua_doc].
    ///
    ...
    ///
    /// Returns the previous value of 'pause'.  More information can be found in
    /// the [Lua 5.4 documentation][lua_doc].
    ///
    /// [lua_doc]: https://www.lua.org/manual/5.4/manual.html#2.5
    ...
    ///
    /// Returns the previous value of the 'step multiplier'.  More information
...
unsafe fn create_lua(lua_mod_to_load : StdLib, init_flags : InitFlags) -> Lua
{
    ...
    ) -> *mut c_void
    {
        ...
        // If the `ptr` argument is null, osize instead encodes the allocated object
        // type, which we currently ignore.
        ...
        else
        {
            ...
            if !p.is_null()
            {
                // Only commit the new used memory if the allocation was successful.  Probably
                // in reality, libc::realloc will never fail.
                (*extra_data).used_memory = new_used_memory;
            }
            p
        }
    ...
    extra.ref_thread = rlua_expect!(
        protect_lua_closure(state, 0, 0, |state| {
            ...
            ffi::lua_pushlightuserdata(state, &FUNCTION_METATABLE_REGISTRY_KEY as *const u8 as *mut c_void);
            ...
            // Override pcall and xpcall with versions that cannot be used to catch rust
            // panics.
            ...
            // Create ref stack thread and place it in the registry to prevent it from being
            // garbage collected.
