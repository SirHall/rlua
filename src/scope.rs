...
/// Constructed by the [`Context::scope`] method, allows temporarily creating
/// Lua userdata and callbacks that are not required to be Send or 'static.
///
...
impl<'lua, 'scope> Scope<'lua, 'scope>
{
    ...
    /// handle to it.
    ///
    /// This is a version of [`Context::create_function`] that creates a
    /// callback which expires on scope drop.  See [`Context::scope`] for
    /// more details.
    ///
    ...
    {
        // Safe, because 'scope must outlive 'callback (due to Self containing 'scope),
        // however the callback itself must be 'scope lifetime, so the function
        // should not be able to capture anything of 'callback lifetime.  'scope
        // can't be shortened due to being invariant, and the 'callback lifetime
        // here can't be enlarged due to coming from a universal quantification
        // in Context::scope.
        //
        // I hope I got this explanation right, but in any case this is tested with
        // compiletest_rs to make sure callbacks can't capture handles with
        // lifetime outside the scope, inside the scope, and owned inside the
        // callback itself.
        unsafe {
    ...
    /// it.
    ///
    /// This is a version of [`Context::create_function_mut`] that creates a
    /// callback which expires on scope drop.  See [`Context::scope`] and
    /// [`Scope::create_function`] for more details.
    ///
    /// [`Context::create_function_mut`]:
    /// struct.Context.html#method.create_function_mut [`Context::scope`]:
    /// struct.Context.html#method.scope [`Scope::create_function`]:
    /// #method.create_function
    pub fn create_function_mut<'callback, A, R, F>(&'callback self, func : F) -> Result<Function<'lua>>
    where
        A : FromLuaMulti<'callback>,
        R : ToLuaMulti<'callback>,
    ...
    ///
    /// This is a version of [`Context::create_userdata`] that creates a
    /// userdata which expires on scope drop, and does not require that the
    /// userdata type be Send (but still requires that the UserData be
    /// 'static).  See [`Context::scope`] for more details.
    ///
    ...
    where
        T : 'static + UserData,
    {
        // Safe even though T may not be Send, because the parent Lua cannot be sent to
        // another thread while the current context is alive.
        unsafe {
    ...
    /// This is a version of [`Context::create_userdata`] that creates a
    /// userdata which expires on scope drop, and does not require that the
    /// userdata type be Send or 'static. See [`Context::scope`] for more
    /// details.
    ///
    /// Lifting the requirement that the UserData type be 'static comes with
    /// some important limitations, so if you only need to eliminate the
    /// Send requirement, it is probably better to use [`Scope::
    /// create_static_userdata`] instead.
    ///
    /// The main limitation that comes from using non-'static userdata is that
    /// the produced userdata will no longer have a `TypeId` associated with
    /// it, becuase `TypeId` can only work for 'static types.  This means
    /// that it is impossible, once the userdata is created, to get a
    /// reference to it back *out* of an `AnyUserData` handle.  This also
    /// implies that the "function" type methods that can be added via
    /// [`UserDataMethods`] (the ones that accept `AnyUserData` as a first
    /// parameter) are vastly less useful.  Also, there is no way to re-use
    /// a single metatable for multiple non-'static types, so there is a higher
    /// cost associated with creating the userdata metatable each time a new
    /// userdata is created.
    ///
        ...
        T : 'scope + UserData,
    {
        ...
        // 'callback outliving 'scope is a lie to make the types work out, required due
        // to the inability to work with the more correct callback type that is
        // universally quantified over 'lua.  This is safe though, because
        // `UserData::add_methods` does not get to pick the 'lua lifetime, so
        // none of the static methods UserData types can add can possibly capture
        // parameters.
        ...
        {
            // On methods that actually receive the userdata, we fake a type check on the
            // passed in userdata, where we pretend there is a unique type per
            // call to `Scope::create_nonstatic_userdata`.  You can grab a
            // method from a userdata and call it on a mismatched userdata type,
            // which when using normal 'static userdata will fail with a type
            // mismatch, but here without this check would proceed as though you had
            // called the method on the original value (since we otherwise completely ignore
            // the first argument).
                ...
                if let Some(value) = value
                {
                    if let Value::UserData(u) = value
                    {
                        unsafe {
                            ...
                            return ffi::lua_touserdata(lua.state, -1) == check_data.as_ptr() as *mut c_void;
                        }
                    }
                }
            ...
            match method
            {
                NonStaticMethod::Method(method) =>
                {
                    ...
                    let f = Box::new(move |lua, mut args : MultiValue<'callback>| {
                        ...
                        let data = method_data.try_borrow().map_err(|_| Error::UserDataBorrowError)?;
                ...
                unsafe { scope.create_callback(function) },
                NonStaticMethod::FunctionMut(function) =>
                {
                    let function = RefCell::new(function);
                    let f = Box::new(move |lua, args| {
                        (&mut *function.try_borrow_mut().map_err(|_| Error::RecursiveMutCallback)?)(lua, args)
                    });
                    unsafe { scope.create_callback(f) }
                },
            }
    ...
    // Unsafe, because the callback can improperly capture any value with 'callback
    // scope, such as improperly capturing an argument. Since the 'callback
    // lifetime is chosen by the user and the lifetime of the callback itself is
    // 'scope (non-'static), the borrow checker will happily pick a 'callback
    // that outlives 'scope to allow this.  In order for this to be safe, the
    // callback must NOT capture any parameters.
    unsafe fn create_callback<'callback>(&self, f : Callback<'callback, 'scope>) -> Result<Function<'lua>>
    {
...
impl<'lua, 'scope> Drop for Scope<'lua, 'scope>
{
    fn drop(&mut self)
    {
        // We separate the action of invalidating the userdata in Lua and actually
        // dropping the userdata type into two phases.  This is so that, in the
        // event a userdata drop panics, we can be sure that all of the userdata
        // in Lua is actually invalidated.
