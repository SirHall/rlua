...
impl<'lua> String<'lua>
{
    ...
    ///
    /// The returned slice will not contain the terminating nul byte, but will
    /// contain any nul bytes embedded into the Lua string.
    ///
    /// # Examples
    ///
    /// ```
    ...
    pub fn as_bytes_with_nul(&self) -> &[u8]
    {
        ...
        unsafe {
            ...
            let mut size = 0;
            // This will not trigger a 'm' error, because the reference is guaranteed to be
            // of string type
...
// Lua strings are basically &[u8] slices, so implement PartialEq for anything
// resembling that.
//
// This makes our `String` comparable with `Vec<u8>`, `[u8]`, `&str`, `String`
// and `rlua::String` itself.
//
// The only downside is that this disallows a comparison with `Cow<str>`, as
// that only implements `AsRef<str>`, which collides with this impl. Requiring
// `AsRef<str>` would fix that, but limit us in other ways.
