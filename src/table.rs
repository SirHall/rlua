...
use crate::util::{assert_stack, geti, objlen, protect_lua, protect_lua_closure, rawlen, StackGuard};
...
impl<'lua> Table<'lua>
{
    ...
    ///
    /// This might invoke the `__newindex` metamethod. Use the [`raw_set`]
    ...
    ///
    /// This might invoke the `__index` metamethod. Use the [`raw_get`] method
    ...
    /// Returns the result of the Lua `#` operator, without invoking the `__len`
    /// metamethod.
    ...
    /// Returns a reference to the metatable of this table, or `None` if no
    /// metatable is set.
    ///
    /// Unlike the `getmetatable` Lua function, this method ignores the
    /// `__metatable` field.
    ...
    ///
    /// If `metatable` is `None`, the metatable is removed (if no metatable is
    ...
    ///
    /// This works like the Lua `pairs` function, but does not invoke the
    /// `__pairs` metamethod.
    ///
    /// The pairs are wrapped in a [`Result`], since they are lazily converted
    /// to `K` and `V` types.
    ///
    /// # Note
    ///
    /// While this method consumes the `Table` object, it can not prevent code
    /// from mutating the table while the iteration is in progress. Refer to
    /// the [Lua manual] for information about the consequences of such
    /// mutation.
    ///
    /// # Examples
    ///
    ...
    /// Consume this table and return an iterator over all values in the
    /// sequence part of the table.
    ///
    /// The iterator will yield all values `t[1]`, `t[2]`, and so on, until a
    /// `nil` value is encountered. This mirrors the behaviour of Lua's
    /// `ipairs` function and will invoke the `__index` metamethod according
    /// to the usual rules. However, the deprecated `__ipairs`
    ...
    /// # fn main() -> Result<()> {
    /// # Lua::new().context(|lua_context| {
    /// let my_table : Table = lua_context
    ///     .load(
    ///         r#"
    ///     {
    ///         [1] = 4,
        ...
        if let Some(next_key) = self.next_key.take()
        {
            ...
            let res = (|| {
                let res = unsafe {
                    ...
                    if protect_lua_closure(lua.state, 2, ffi::LUA_MULTRET, |state| ffi::lua_next(state, -2) != 0)?
                    {
