...
// Checks that Lua has enough free stack space for future stack operations.  On
// failure, this will panic with an internal error message.
pub unsafe fn assert_stack(state : *mut ffi::lua_State, amount : c_int)
{
    // TODO: This should only be triggered when there is a logic error in `rlua`.
    // In the future, when there is a way to be confident about stack safety and
    // test it, this could be enabled only when `cfg!(debug_assertions)` is
    // true.
    rlua_assert!(ffi::lua_checkstack(state, amount) != 0, "out of stack space");
}

// Checks that Lua has enough free stakc space and returns `Error::StackError`
// on failure.
...
impl StackGuard
{
    // Creates a StackGuard instance with wa record of the stack size, and on Drop
    // will check the stack size and drop any extra elements.  If the stack size
    // at the end is *smaller* than at the beginning, this is considered a fatal
    // logic error and will result in a panic.
...
// Call a function that calls into the Lua API and may trigger a Lua error
// (longjmp) in a safe way. Wraps the inner function in a call to `lua_pcall`,
// so the inner function only has access to a limited lua stack.  `nargs` is the
// same as the the parameter to `lua_pcall`, and `nresults` is
// always LUA_MULTRET.  Internally uses 2 extra stack spaces, and does not call
// checkstack. Provided function must *never* panic.
...
// Call a function that calls into the Lua API and may trigger a Lua error
// (longjmp) in a safe way. Wraps the inner function in a call to `lua_pcall`,
// so the inner function only has access to a limited lua stack.  `nargs` and
// `nresults` are similar to the parameters of `lua_pcall`, but the
// given function return type is not the return value count, instead the inner
// function return values are assumed to match the `nresults` param.  Internally
// uses 3 extra stack spaces, and does not call checkstack.  Provided function
// must *not* panic, and since it will generally be longjmping, should not
// contain any values that implement Drop.
    ...
    R : Copy,
{
    ...
    if ret == ffi::LUA_OK
    {
        // LUA_OK is only returned when the do_call function has completed successfully,
        // so params.result is definitely initialized.
...
// Pops an error off of the stack and returns it.  The specific behavior depends
// on the type of the error at the top of the stack:
//   1) If the error is actually a WrappedPanic, this will continue the panic.
//   2) If the error on the top of the stack is actually a WrappedError, just
// returns it.   3) Otherwise, interprets the error as the appropriate lua
// error. Uses 2 stack spaces, does not call lua_checkstack.
...
pub unsafe fn push_string<S : ?Sized + AsRef<[u8]>>(state : *mut ffi::lua_State, s : &S) -> Result<()>
{
...
// Pops the userdata off of the top of the stack and returns it to rust,
// invalidating the lua userdata and gives it the special "destructed" userdata
// metatable.  Userdata must not have been previously invalidated, and this
// method does not check for this.  Uses 1 extra stack space and does not call
// checkstack
pub unsafe fn take_userdata<T>(state : *mut ffi::lua_State) -> T
{
    // We set the metatable of userdata on __gc to a special table with no __gc
    // method and with metamethods that trigger an error on access.  We do this
    // so that it will not be double dropped, and also so that it cannot be used
    // or identified as any particular userdata type after the first call to
    // __gc.
...
// Populates the given table with the appropriate members to be a userdata
// metatable for the given type.  This function takes the given table at the
// `metatable` index, and adds an appropriate __gc member to it for the given
// type and a __metatable entry to protect the table from script access.
// The function also, if given a `members` table index, will set up an __index
// metamethod to return the appropriate member on __index.  Additionally, if
// there is already an __index entry on the given metatable, instead of simply
// overwriting the __index, instead the created __index method will capture the
// previous one, and use it as a fallback only if the given key is not found in
// the provided members table.  Internally uses 6 stack spaces and does not call
// checkstack.
...
{
    // Used if both an __index metamethod is set and regular methods, checks methods
    // table first, then __index metamethod.
...
#[cfg(rlua_lua54)]
// Wrapper around ffi::lua_getiuservalue or ffi::lua_getuservalue depending on
// the Lua version.
...
#[cfg(rlua_lua51)]
// Wrapper around ffi::lua_getiuservalue, ffi::lua_getuservalue depending on the
// Lua version. On Lua 5.1 this is emulated using ffi::lua_getfenv; the index
// must be 1.
pub unsafe fn getiuservalue(state : *mut ffi::lua_State, index : c_int, n : c_int) -> c_int
{
    if n != 1
...
#[cfg(rlua_lua54)]
// Wrapper around ffi::lua_setiuservalue or ffi::lua_setuservalue depending on
// the Lua version.
...
#[cfg(rlua_lua51)]
// Wrapper around ffi::lua_setiuservalue or ffi::lua_setuservalue depending on
// the Lua version. On Lua 5.1 this maps to ffi::lua_setfenv; index must be 1
// and the value must be a table.
pub unsafe fn setiuservalue(state : *mut ffi::lua_State, index : c_int, n : c_int) -> c_int
{
    if n != 1
    {
...
pub unsafe fn push_globaltable(state : *mut ffi::lua_State) { ffi::lua_pushvalue(state, ffi::LUA_GLOBALSINDEX); }
...
pub unsafe fn tointegerx(state : *mut ffi::lua_State, index : c_int, isnum : *mut c_int) -> ffi::lua_Integer
{
    ...
    {
        // Lua 5.1 happily truncates non-integral floats, but rlua currently expects the
        // conversion to fail as Lua 5.3+ do.
...
#[cfg(rlua_lua51)]
pub unsafe fn rawlen(state : *mut ffi::lua_State, index : c_int) -> usize { ffi::lua_objlen(state, index) }
...
{
    let load_result = loadbufferx(state, s.as_ptr() as *const c_char, s.len(), cstr!(""), cstr!("t"));
...
pub unsafe fn requiref(state : *mut ffi::lua_State, modname : *const c_char, openf : ffi::lua_CFunction, glb : c_int)
{
...
pub unsafe fn requiref(state : *mut ffi::lua_State, modname : *const c_char, openf : ffi::lua_CFunction, _glb : c_int)
{
...
pub unsafe fn tolstring(state : *mut ffi::lua_State, index : c_int, len : *mut usize) -> *const c_char
{
...
#[cfg(rlua_lua51)]
pub unsafe fn dump(state : *mut ffi::lua_State, writer : ffi::lua_Writer, data : *mut c_void, _strip : c_int) -> c_int
{
...
// will result in a call to lua_error (a longjmp).  The error or panic is
// wrapped in such a way that when calling pop_error back on the rust side, it
// will resume the panic.
//
// This function assumes the structure of the stack at the beginning of a
// callback, that the only elements on the stack are the arguments to the
// callback.
//
// This function uses some of the bottom of the stack for error handling, the
// given callback will be given the number of arguments available as an
// argument, and should return the number of returns as normal, but cannot
// assume that the arguments available start at 0.
    ...
    F : FnOnce(c_int) -> Result<R>,
{
    ...
    // We need one extra stack space to store preallocated memory, and at least 3
    ...
    // We cannot shadow rust errors with Lua ones, we pre-allocate enough memory to
    // store a wrapped error or panic *before* we proceed.
...
pub unsafe extern "C" fn error_traceback(state : *mut ffi::lua_State) -> c_int
{
    ...
    {
        // lua_newuserdatauv and luaL_traceback may error, but nothing that implements
        // Drop should be on the rust stack at this time.
        // We don't need any user values in this userdata
...
// A variant of pcall that does not allow lua to catch panic errors from
// callback_error
...
// A variant of xpcall that does not allow lua to catch panic errors from
// callback_error
...
// Pushes a WrappedError to the top of the stack.  Uses two stack spaces and
// does not call lua_checkstack.
...
// Checks if the value at the given index is a WrappedError, and if it is
// returns a pointer to it, otherwise returns null.  Uses 2 stack spaces and
// does not call lua_checkstack.
...
pub unsafe fn init_error_registry(state : *mut ffi::lua_State, wrap_panics : bool)
{
    ...
    unsafe extern "C" fn error_tostring(state : *mut ffi::lua_State) -> c_int
    {
        let err_buf = callback_error(state, |_| {
            ...
            if let Some(error) = get_wrapped_error(state, -1).as_ref()
            {
                ...
                // memory (for example, some kind of recursive error structure?)
        ...
        ffi::lua_pushlstring(state, (*err_buf).as_ptr() as *const c_char, (*err_buf).len());
        (*err_buf).clear();
        1
    }

    ffi::lua_pushlightuserdata(state, &ERROR_METATABLE_REGISTRY_KEY as *const u8 as *mut c_void);
    ...
    if wrap_panics
    {
        ffi::lua_pushlightuserdata(state, &PANIC_METATABLE_REGISTRY_KEY as *const u8 as *mut c_void);
        ffi::lua_newtable(state);
    ...
    ffi::lua_pushlightuserdata(state, &DESTRUCTED_USERDATA_METATABLE as *const u8 as *mut c_void);
    ffi::lua_newtable(state);
...
// Converts the given lua value to a string in a reasonable format without
...
unsafe fn to_string<'a>(state : *mut ffi::lua_State, index : c_int) -> Cow<'a, str>
{
    match ffi::lua_type(state, index)
    {
        ...
        ffi::LUA_TLIGHTUSERDATA => format!("<lightuserdata {:?}>", ffi::lua_topointer(state, index)).into(),
        ffi::LUA_TNUMBER =>
        {
            let mut isint = 0;
...
// Checks if the value at the given index is a WrappedPanic.  Uses 2 stack
// spaces and does not call lua_checkstack.
