...
/// A dynamically typed Lua value.  The `String`, `Table`, `Function`, `Thread`,
/// and `UserData` variants contain handle types into the internal Lua state.
/// It is a logic error to mix handle types between separate `Lua` instances, or
/// between a parent `Lua` instance and one received as a parameter in a Rust
...
pub enum Value<'lua>
{
    ...
    /// An integer number.
    ///
    /// Any Lua number convertible to a `Integer` will be represented as this
    /// variant. (Lua 5.3+ only)
    Integer(Integer),
    ...
    /// `Error` is a special builtin userdata type.  When received from Lua it
...
/// Multiple Lua values used for both argument passing and also for multiple
/// return values.
#[derive(Debug, Clone)]
...
impl<'lua> MultiValue<'lua>
{
    ...
    pub fn new() -> MultiValue<'lua> { MultiValue(Vec::new()) }
}

impl<'lua> Default for MultiValue<'lua>
{
    fn default() -> MultiValue<'lua> { MultiValue::new() }
}

impl<'lua> FromIterator<Value<'lua>> for MultiValue<'lua>
{
    fn from_iter<I : IntoIterator<Item = Value<'lua>>>(iter : I) -> Self { MultiValue::from_vec(Vec::from_iter(iter)) }
}
...
impl<'lua> MultiValue<'lua>
{
    ...
    pub(crate) fn push_front(&mut self, value : Value<'lua>) { self.0.push(value); }

    pub(crate) fn pop_front(&mut self) -> Option<Value<'lua>> { self.0.pop() }

    pub fn len(&self) -> usize { self.0.len() }

    pub fn is_empty(&self) -> bool { self.0.len() == 0 }

    pub fn iter(&self) -> iter::Rev<slice::Iter<Value<'lua>>> { self.0.iter().rev() }
}
...
pub trait FromLuaMulti<'lua>: Sized
{
    ...
    /// semantics of Lua when calling a function or assigning values.
    /// Similarly, if not enough values are given, conversions should assume
    /// that any missing values are nil.
